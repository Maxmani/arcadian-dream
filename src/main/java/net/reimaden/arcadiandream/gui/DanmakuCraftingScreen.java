...
public class DanmakuCraftingScreen extends HandledScreen<DanmakuCraftingScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        renderIcons(matrices, x, y);
    }

    private void renderIcons(MatrixStack matrices, int x, int y) {
        ...
        for (int i = 0; i < iconPositions.length; i++) {
            ...
            if (slotIsEmpty(slotIndex)) {
                drawTexture(matrices, x + position[0], y + position[1], position[2], position[3], 16, 16);
            }
        }
    }
